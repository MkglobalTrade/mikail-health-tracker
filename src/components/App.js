...
import { lsGet, lsSet, today, getStatus } from '../lib/data'
...
export default function App() {
  ...
  const addLab   = entry => { const d = [entry, ...labs];  lsSet('mk_labs_v9',  d); setLabs(d);  flash('✓ Result saved to history') }
  ...
  const flash = (msg, cb) => { setToast(msg); setTimeout(() => { setToast(''); cb?.() }, 1600) }

  const L = labs[0] || {}
  const crits = LABS.filter(f => L[f.key] && getStatus(f, L[f.key]) === 'crit')

  const headerProps = {
    home:    { title: 'Mikail Kocak', subtitle: '46 yrs · Los Angeles, CA', right: crits.length > 0 ? <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '4px 10px', fontSize: 11, color: '#DC2626', fontWeight: 700 }}>⚠ {crits.length} critical</div> : null },
  ...
  const hp = headerProps[tab] || headerProps.home
