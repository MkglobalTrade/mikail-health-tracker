...
function InfoRow({ icon, text, color, bold, italic }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, color:color||(italic?'#9CA3AF':'#374151'), marginBottom:5, fontWeight:bold?600:400, fontStyle:italic?'italic':'normal' }}>
...
function InsuranceSection() {
  ...
  if (!data.provider) {
    return (
      <div onClick={() => setEdit(true)} style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:14, padding:'14px 16px', cursor:'pointer' }}>
    ...
    <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:14, padding:'14px 16px' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          ...
          {data.plan && <InfoRow icon="📋" text={data.plan} />}
          ...
          {data.notes && <InfoRow icon="📝" text={data.notes} italic />}
        </div>
...
export default function DoctorsScreen() {
  ...
  const SPEC_C = { Nephrologist:'#0891B2', Cardiologist:'#DC2626', Endocrinologist:'#7C3AED', 'Primary Care':'#16A34A' }
  const sc = spec => SPEC_C[spec] || '#4F46E5'

  return (
    <div style={{ padding:'16px 16px 0' }}>
      <div style={{ fontSize:12, color:'#6B7280', marginBottom:16, lineHeight:1.6 }}>
        Your doctors and hospitals — always available. Tap any phone number to call directly.
      </div>
            ...
            <div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:10, flex:1 }}>
                  <div style={{ width:44, height:44, borderRadius:14, background:'#EEF2FF', display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, fontWeight:700, color:sc(c.specialty), border:'1px solid #C7D2FE', flexShrink:0 }}>
                    {initials(c)}
                  </div>
                  <div>
                    <div style={{ fontSize:15, fontWeight:600, color:c.name?'#1F2937':'#9CA3AF' }}>{c.name||'No name yet'}</div>
                    <div style={{ fontSize:12, color:sc(c.specialty), fontWeight:500 }}>{c.specialty||'Specialty'}</div>
                  </div>
                </div>
              ...
              {c.notes    && <InfoRow icon="📝" text={c.notes} italic />}
              {!c.name && !c.hospital && !c.phone && <div style={{ fontSize:12, color:'#D1D5DB', fontStyle:'italic' }}>Tap ✎ to add information</div>}
            </div>
          )}
        </div>
      ))}
