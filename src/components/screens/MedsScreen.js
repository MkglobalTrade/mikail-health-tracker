...
const T_LABEL = { morning: '🌅 Morning', night: '🌙 Night' }
...
export default function MedsScreen() {
  ...
  const all = [...(meds.morning || []), ...(meds.night || [])]
  ...
  const saveMed = () => {
    ...
    const t = form.time || view, other = t === 'morning' ? 'night' : 'morning'
    let updated = { ...meds }
    if (modal === 'edit') {
      const inT = (updated[t] || []).some(m => m.id === form.id)
      updated = {
        [t]:     inT ? (updated[t] || []).map(m => m.id === form.id ? { ...form } : m) : [...(updated[t] || []), { ...form }],
        [other]: (updated[other] || []).filter(m => m.id !== form.id)
      }
    } else {
  ...
  const tabBtn = (t) => (
    <button key={t} onClick={() => setView(t)} style={{
      ...
      background: view === t ? '#EEF2FF' : 'transparent',
      borderBottom: `2px solid ${view === t ? '#4F46E5' : 'transparent'}`,
      ...
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, padding: '14px 16px', marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#6B7280', marginTop: 2 }}>{done} of {all.length} taken</div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 300, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', padding: '22px 18px 40px', width: '100%', maxWidth: 430, maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              ...
              {modal === 'edit' && (
                <button onClick={() => { delMed(form.id, form.time); setModal(null) }} style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 12, padding: '13px 16px', color: '#DC2626', fontSize: 14, fontWeight: 500 }}>Delete</button>
              )}
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 16px', borderBottom: isLast ? 'none' : '1px solid #F9FAFB', background: done ? '#FAFAFA' : '#fff' }}>
      ...
      <div style={{ flex: 1, cursor: 'pointer' }} onClick={onToggle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 2 }}>
          ...
          {m.dose && m.dose !== '—' && <span style={{ fontSize: 10, padding: '1px 7px', borderRadius: 20, background: '#EEF2FF', color: '#3730A3', border: '1px solid #C7D2FE', fontWeight: 500 }}>{m.dose}</span>}
