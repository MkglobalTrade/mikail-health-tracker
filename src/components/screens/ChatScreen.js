  ...
  const send = async (text) => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: updated, context: buildContext(labs, stelo) })
      })
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 8px' }}>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ marginBottom: 12, display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
            ...
            <div style={{
              maxWidth: '85%', padding: '10px 13px', borderRadius: m.role === 'user' ? '14px 14px 3px 14px' : '14px 14px 14px 3px',
              background: m.role === 'user' ? '#4F46E5' : '#F9FAFB',
              color: m.role === 'user' ? '#fff' : '#1F2937',
              fontSize: 13, lineHeight: 1.6, border: m.role === 'assistant' ? '1px solid #E5E7EB' : 'none'
            }}>
              {m.content.split('\n').map((line, j) => (
          ...
          <div style={{ marginTop: 8 }}>
            ...
            {SUGGESTIONS.map((s, i) => (
              <button key={i} onClick={() => send(s)} style={{
                ...
                padding: '9px 12px', marginBottom: 6, fontSize: 12, color: '#374151',
                cursor: 'pointer'
              }}>
                {s}
              </button>
      ...
      <div style={{ padding: '8px 16px 16px', background: '#fff', borderTop: '1px solid #E5E7EB' }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          <textarea
            ...
            style={{ flex: 1, background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 12, padding: '10px 13px', fontSize: 13, resize: 'none', height: 42, outline: 'none', lineHeight: 1.4 }}
          />
