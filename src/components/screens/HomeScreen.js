...
import { LABS, STELO, CAT_ORDER, CAT_COLOR, getStatus, STATUS_C, STATUS_BG, gc, normalRange } from '../../lib/data'
import { SectionCard, Chip, Bar, Spark } from '../ui'

function TrendChart({ labs, stelo }) {
  const [sel, setSel] = useState('hba1c')

  const allFields = [...LABS, ...STELO]
  const field = allFields.find(f => f.key === sel)
  const steloKeys = ['glucose_fasting','glucose_peak','glucose_avg','time_in_range']
  const src = steloKeys.includes(sel) ? stelo : labs
  const data = [...src].reverse().filter(e => e[sel] != null).map(e => ({ date: e.date, val: parseFloat(e[sel]) }))

  const available = allFields.filter(f => {
    const s = steloKeys.includes(f.key) ? stelo : labs
    ...
    <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:16, padding:16, marginBottom:12 }}>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginBottom:12 }}>
        {available.slice(0, 8).map(f => (
          <button key={f.key} onClick={() => setSel(f.key)} style={{ padding:'3px 10px', borderRadius:20, fontSize:10, fontWeight:500, border:'1px solid '+(sel===f.key?(CAT_COLOR[f.cat]||'#4F46E5'):'#E5E7EB'), background:sel===f.key?(CAT_COLOR[f.cat]||'#4F46E5'):'#fff', color:sel===f.key?'#fff':'#6B7280' }}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {data.length < 2 ? (
        ...
        <>
          <svg width="100%" viewBox={'0 0 '+W+' '+H} style={{ display:'block' }}>
            {field?.hi && (
              <line x1={WP} x2={W-WP} y1={TP+(1-(field.hi-mn)/rng)*(H-TP-BP)} y2={TP+(1-(field.hi-mn)/rng)*(H-TP-BP)} stroke="#E5E7EB" strokeWidth="1" strokeDasharray="4,3" />
            )}
            ...
            {pts.map((p, i) => (
              <g key={i}>
                ...
                <text x={p.x} y={H-4} textAnchor="middle" fontSize="8" fill="#9CA3AF">{p.d.date?.slice(5)}</text>
              </g>
            ))}
          ...
          <div style={{ display:'flex', gap:8, marginTop:10, paddingTop:10, borderTop:'1px solid #F3F4F6' }}>
            <div style={{ flex:1, background:'#F9FAFB', borderRadius:10, padding:'8px 10px' }}>
              <div style={{ fontSize:9, color:'#9CA3AF', marginBottom:2 }}>FIRST</div>
              <div style={{ fontSize:15, fontWeight:600 }}>{data[0].val}</div>
            </div>
            <div style={{ flex:1, background:'#ECFDF5', borderRadius:10, padding:'8px 10px' }}>
              <div style={{ fontSize:9, color:'#9CA3AF', marginBottom:2 }}>LATEST</div>
              <div style={{ fontSize:15, fontWeight:600, color:gc(field, data[data.length-1].val) }}>{data[data.length-1].val}</div>
            </div>
  ...
  if (labs.length === 0 && stelo.length === 0) {
    return (
      <div style={{ textAlign:'center', padding:'72px 24px' }}>
        <div style={{ fontSize:52, marginBottom:16, opacity:.1 }}>🩺</div>
      ...
      {CAT_ORDER.map(cat => {
        ...
        return (
          <SectionCard key={cat} title={cat} color={cc} date={L.date}>
            {fields.map(f => {
              const v = L[f.key]
              const s = getStatus(f, v)
              const sc = STATUS_C[s]
              const h = hist(f.key)
              return (
                <div key={f.key} style={{ display:'flex', alignItems:'center', padding:'9px 0', borderBottom:'1px solid #F9FAFB', gap:12 }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div>
                      <span style={{ fontSize:20, fontWeight:600, color:sc }}>{v}</span>
