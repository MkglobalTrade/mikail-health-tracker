...
import { LABS, STELO, CAT_ORDER, CAT_COLOR, getStatus, STATUS_COLOR, STATUS_BG, gc } from '../../lib/data'

export default function HistoryScreen({ labs, stelo, onDeleteLab, onDeleteStelo }) {
  const [exp, setExp] = useState(null)

  const byMonth = {}
  ;[...labs.map(e => ({ ...e, _t: 'lab' })), ...stelo.map(e => ({ ...e, _t: 'stelo' }))]
    .forEach(e => {
      ...
      {Object.keys(byMonth).sort((a, b) => b.localeCompare(a)).map(month => (
        <div key={month} style={{ marginBottom: 20 }}>
          ...
          {byMonth[month].sort((a, b) => (b.date || '').localeCompare(a.date || '')).map(entry => {
            ...
            const warnCount = labFields.filter(f => getStatus(f, entry[f.key]) === 'warn').length
                        ...
                        {CAT_ORDER.map(cat => {
                          ...
                          return (
                            <div key={cat} style={{ marginBottom: 12 }}>
                              <div style={{ fontSize: 9, fontWeight: 700, color: cc, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 7 }}>{cat}</div>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                                {fields.map(f => {
                                  const s = getStatus(f, entry[f.key])
                                  return (
                                    <div key={f.key} style={{ background: STATUS_BG[s], borderRadius: 10, padding: '7px 11px', borderLeft: `3px solid ${STATUS_COLOR[s]}` }}>
                                      <div style={{ fontSize: 9, color: '#9CA3AF', marginBottom: 2 }}>{f.label}</div>
                                      <div style={{ fontSize: 14, fontWeight: 600, color: STATUS_COLOR[s] }}>
                                        {entry[f.key]} <span style={{ fontSize: 8, opacity: .5 }}>{f.unit}</span>
                                      </div>
                                    </div>
                                  )
                                })}
