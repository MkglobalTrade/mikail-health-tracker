...
Return ONLY valid JSON — no markdown, no explanation, nothing else before or after.

{
  ...
  "values": {
    ...
    "sodium":null,"potassium":null,"alt":null,"ast":null,"albumin":null,
...
Rules: JSON only. No extra text. HbA1c over 14 use 14.1. Glucose over 1000 use 1001.
Handles Albanian, English, Turkish, Spanish documents automatically.`
...
export async function POST(req) {
  try {
    ...
    if (!key) {
      return Response.json({ ok: false, error: 'ANTHROPIC_API_KEY not set in Vercel environment variables' }, { status: 500 })
    }
    const content = imageData
      ? [
          { type: 'image', source: { type: 'base64', media_type: imageMime, data: imageData } },
          { type: 'text', text: 'Extract all lab values. Return JSON only.' }
        ]
