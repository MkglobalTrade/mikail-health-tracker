...
Rules: JSON only. HbA1c>14 use 14.1. Glucose>1000 use 1001. Works for Albanian, English, Turkish, Spanish.`

export async function POST(req) {
  try {
    const { imageData, imageMime, text } = await req.json()
    const key = process.env.ANTHROPIC_API_KEY
    if (!key) return Response.json({ ok: false, error: 'ANTHROPIC_API_KEY not configured in Vercel environment variables' }, { status: 500 })
